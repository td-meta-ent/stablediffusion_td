...
CONFIG_PATH = os.path.join(PROJECT_PATH,"configs/stable-diffusion/v2-midas-inference.yaml")
CHECKPOINT_PATH = os.path.join(PROJECT_PATH,"weight/512-depth-ema.ckpt")
...
# def run():
st.title("Stable Diffusion Depth2Image")
st.markdown("""
    ## If size of input image is bigger than (1024,1024), the image will resize to (1024,1024)
""")
video_file = open('/home/td/Project/stablediffusion/scripts/samples/depth2image.mp4', 'rb')
...
if image:
    ...
    scale = st.slider("Scale", min_value=0.1, max_value=30.0, value=20.0, step=0.1)
