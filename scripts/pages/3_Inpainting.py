...
CONFIG_PATH = os.path.join(PROJECT_PATH,"configs/stable-diffusion/v2-inpainting-inference.yaml")
CHECKPOINT_PATH = os.path.join(PROJECT_PATH,"weight/512-inpainting-ema.ckpt")
...
""")
video_file = open('/home/td/Project/stablediffusion/scripts/samples/inpainting.mp4', 'rb')
video_bytes = video_file.read()
st.video(video_bytes)

sampler = initialize_model(CONFIG_PATH, CHECKPOINT_PATH)
image=None
...
if image:
    ...
    scale = st.slider("Scale", min_value=0.1, max_value=30.0, value=20., step=0.1)
    ...
    st.write("Canvas")
    st.caption(
        "Draw a mask to inpaint, then click the 'Send to Streamlit' button (bottom left, with an arrow on it). Reload image if image and canvas not match")
